...
__global__ void transpose(float *input, float *output, int rows, int cols) {
  ...
  size_t groupCol = blockIdx.x;
  size_t groupRow = blockIdx.y;
  size_t localCol = threadIdx.x;
  size_t localRow = threadIdx.y;
  size_t globalCol = groupCol * blockDim.x + localCol;
  size_t globalRow = groupRow * blockDim.y + localRow;
  ...
  size_t outRow = groupCol * blockDim.x + localRow;
  size_t outCol = groupRow * blockDim.y + localCol;
  if (outRow < cols && outCol < rows)
    output[outRow * cols + outCol] = tile[localCol][localRow];
}
...
int main(int argc, char **argv) {
  ...
  dim3 threads(tileLength, tileLength);
  dim3 blocks((cols + tileLength - 1) / tileLength,
